...
**Independent Test**: Can be fully tested by providing a alignment run with known incorrect timestamps, allowing the user to edit specific verse start/end times, validating the edits maintain constraints, and saving the corrected version. Delivers immediate value by making previously unusable alignments production-ready.

**Acceptance Scenarios**:

1. **Given** a alignment run with verse 3 incorrectly ending at 12.3s (should be 11.8s), **When** user loads the file for correction and adjusts verse 3 end time to 11.8s, **Then** the system validates the change, auto-adjusts verse 4 start time to 11.8s to maintain continuity, and saves the corrected timestamps with a backup of the original
2. **Given** an AlignmentRun with low confidence score (0.72) on verse 5, **When** user reviews the timestamps, **Then** the system highlights low-confidence verses and allows targeted correction
...
As a Torah educator with many verses to correct, when I need to adjust multiple timestamps efficiently, I want to export the AlignmentRun to CSV format, edit it in a spreadsheet tool (Excel, Google Sheets), and import the corrected version so that I can make bulk edits faster than one-by-one corrections.
...
**Independent Test**: Can be fully tested by exporting an AlignmentRun to CSV, verifying all data is present (reference, hebrew_text, start_time, end_time, confidence), editing the CSV externally, importing it back, and validating all constraints are enforced. Delivers value by enabling batch operations.

**Acceptance Scenarios**:

1. **Given** a alignment run with 8 verses, **When** user exports to CSV format, **Then** the system creates a CSV file with columns: reference, hebrew_text, start_time, end_time, confidence, with UTF-8 encoding preserved for Hebrew text
...
**Independent Test**: Can be fully tested by correcting an AlignmentRun, marking it as manually corrected, re-running the pipeline on the same audio file, and verifying the pipeline skips alignment and uses the corrected timestamps directly. Delivers value by making corrections permanent.
...
2. **Given** a manually corrected AlignmentRun marked as "manually_corrected: true", **When** user runs pipeline with --force-realign flag, **Then** the system creates a new automated alignment (overwriting the manual corrections) and warns the user
...
**Independent Test**: Can be fully tested by processing an audio file that produces alignments with varying confidence scores (some >0.90, some <0.90), displaying the AlignmentRun with confidence indicators, and allowing users to sort/filter by confidence. Delivers value by triaging correction work.
...
2. **Given** an AlignmentRun with 8 verses where verses 2, 5, and 7 have confidence <0.85, **When** user loads the file for correction, **Then** the system highlights these verses and allows "jump to next low-confidence verse" navigation
3. **Given** user corrects all low-confidence verses, **When** user saves, **Then** the system recalculates overall confidence and marks the AlignmentRun as "manually_corrected: true"

---

### User Story 5 - Restore from Backup (Priority: P3)

As a Torah educator who accidentally made incorrect edits to an AlignmentRun, when I realize my corrections were wrong, I want to restore the original automated alignment from backup so that I can start over without re-running the alignment process.
...
**Independent Test**: Can be fully tested by correcting an AlignmentRun (which creates a backup), making additional edits that are incorrect, and restoring from the backup file to return to the original state. Delivers value by providing undo functionality.

**Acceptance Scenarios**:

1. **Given** user has corrected an AlignmentRun (backup created automatically), **When** user runs restore command with the backup file path, **Then** the system replaces the current AlignmentRun with the backup and marks it as "manually_corrected: false"
...
- What happens when user tries to correct an AlignmentRun while video is being rendered? System should detect file lock and warn "File in use by rendering process, wait for completion"
...
- What happens when user tries to restore from a backup that doesn't match current AlignmentRun structure (old schema version)? System should reject with error "Backup schema incompatible, cannot restore"

## Requirements *(mandatory)*

### Functional Requirements

- **FR-020**: System MUST load existing AlignmentRun (with PasukAlignments) from DuckDB for review and editing
...
- **FR-024**: System MUST automatically create a backup of the original AlignmentRun before saving any corrections
- **FR-025**: System MUST mark corrected AlignmentRuns with metadata: `manually_corrected` flag, `corrected_at` timestamp, `backup_path`, `original_quality`. Per-verse corrections tracked via `pasuk_alignments.manually_corrected` and `original_start_time`/`original_end_time`
- **FR-026**: System MUST export PasukAlignments to CSV format with columns: reference, hebrew_text, start_time, end_time, confidence
...
- **FR-028**: System MUST integrate with the processing pipeline to detect existing corrected AlignmentRuns and skip automated alignment
- **FR-029**: System MUST allow users to restore AlignmentRuns from backup files
...
- **FR-033**: System MUST highlight low-confidence verses (confidence <0.85) when displaying AlignmentRuns for correction
...
- **AlignmentRun**: Stores alignment run metadata including `manually_corrected` flag, `corrected_at` timestamp, `backup_path`, and `original_quality`. Links to `alya_audio` (the source audio). See `tmp/db_erd.md`.
- **PasukAlignment**: Individual verse alignment row within an alignment run. Tracks `original_start_time`/`original_end_time` for pre-correction values and `manually_corrected` per verse.
- **ValidationResult**: Transient model — outcome of timestamp validation including pass/fail status, constraint violations, and affected verse references
- **BackupRecord**: Transient model — backup file path, creation timestamp, and description

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-010**: Users can load, correct, and save an AlignmentRun with 10 verses in under 5 minutes
- **SC-011**: 100% of constraint violations are detected during validation (no invalid AlignmentRuns can be saved)
...
- **SC-013**: Manually corrected AlignmentRuns are automatically used by the pipeline with zero additional user actions required
